...
- Tests: pytest + doctest configured in `pyproject.toml`. Run using: `uv run -m pytest [<filename>]`.
- Lint/typecheck: Ruff and Pyright configured in `pyproject.toml`. Check using `uv run ruff check --fix [<filename>]`. PyLance is used in-editor for strict type checking.
...
- Type checking: Respect Ruff and Pyright rules and check results.
